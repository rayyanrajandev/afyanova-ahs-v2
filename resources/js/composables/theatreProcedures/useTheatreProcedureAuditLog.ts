...
/**
 * Matches TheatreProcedureAuditLogResponseTransformer::transform() exactly
 * — unlike lab/radiology's audit log transformers, this one does NOT
 * include the FK field (no `theatreProcedureId` key at all).
 */
export type TheatreProcedureAuditLog = {
    id: string;
    actorId: number | null;
...
export function useTheatreProcedureAuditLog(
...
): AuditLogQueryResult<TheatreProcedureAuditLog> {
    ...
    const query = useQuery({
        queryKey: [
            'theatre-procedure-audit-logs',
            computed(() => toValue(orderId)),
            computed(() => ({ ...filters })),
        ],
        queryFn: async () => {
            const id = toValue(orderId);
            return apiGet<AuditLogListEnvelope>(`/theatre-procedures/${id}/audit-logs`, {
    ...
    function exportCsv(): void {
        ...
        const url = new URL(`${API_V1_PREFIX}/theatre-procedures/${id}/audit-logs/export`, window.location.origin);
