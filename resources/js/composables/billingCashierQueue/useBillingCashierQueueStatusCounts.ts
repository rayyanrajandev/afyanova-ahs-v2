 ...
 * Three small requests instead of one, but each is cheap (no invoice/order
 * data materialized beyond building patient IDs).
 */
export function useBillingCashierQueueStatusCounts(
    ...
    return useQuery({
        queryKey: ['billing-cashier-queue-status-counts', computed(() => ({ q: filters.q, status: filters.status }))],
        queryFn: async () => {
