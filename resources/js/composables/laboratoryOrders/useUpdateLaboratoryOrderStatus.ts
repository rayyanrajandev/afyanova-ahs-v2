...
import type { LaboratoryOrder, LaboratoryOrderStatus } from './useLaboratoryOrders';
...
> {
    return useMutation({
        mutationFn: async ({ id, ...payload }: UpdateLaboratoryOrderStatusPayload): Promise<LaboratoryOrder> => {
            const response = await apiPatch<UpdateLaboratoryOrderStatusResponse>(`/laboratory-orders/${id}/status`, { body: payload });
