...
export type ClinicalProcedureOrderAuditLog = {
    id: string;
    clinicalProcedureOrderId: string | null;
    actorId: number | null;
...
type AuditLogListEnvelope = {
    data: ClinicalProcedureOrderAuditLog[];
    meta: { currentPage: number; perPage: number; total: number; lastPage: number };
};

export function useClinicalProcedureOrderAuditLog(
    orderId: MaybeRefOrGetter<string | null | undefined>,
): AuditLogQueryResult<ClinicalProcedureOrderAuditLog> {
    ...
    const query = useQuery({
        queryKey: [
            'clinical-procedure-order-audit-logs',
            computed(() => toValue(orderId)),
            computed(() => ({ ...filters })),
        ],
        queryFn: async () => {
            const id = toValue(orderId);
            return apiGet<AuditLogListEnvelope>(`/clinical-procedure-orders/${id}/audit-logs`, {
    ...
    function exportCsv(): void {
        ...
        const url = new URL(`${API_V1_PREFIX}/clinical-procedure-orders/${id}/audit-logs/export`, window.location.origin);
