...
/**
 * AdmB of the Admission V2 full-parity plan — mirrors
 * useEmergencyCaseAuditLog.ts exactly (same filters/query/exportCsv
 * shape), hitting GET /admissions/{id}/audit-logs +
 * .../audit-logs/export, so the same shared AuditLogSheet.vue serves this
 * domain via the AuditLogQueryResult contract.
 */
export function useAdmissionAuditLog(admissionId: MaybeRefOrGetter<string | null | undefined>): AuditLogQueryResult<AdmissionAuditLog> {
    ...
    const query = useQuery({
        queryKey: [
            ...
            computed(() => toValue(admissionId)),
            computed(() => ({ ...filters })),
        ],
        queryFn: async () => {
            const id = toValue(admissionId);
            return apiGet<AuditLogListEnvelope>(`/admissions/${id}/audit-logs`, {
        ...
        enabled: computed(() => Boolean(toValue(admissionId))),
    });
    ...
    function exportCsv(): void {
        ...
        const url = new URL(`${API_V1_PREFIX}/admissions/${id}/audit-logs/export`, window.location.origin);
