...
type InventoryCategoryTemplate = 'expiry_sensitive' | 'specialist_equipment' | 'general_supply';
...
const { permissionNames: sharedPermissionNames, isFacilitySuperAdmin, hasPermission, permissionState } = usePlatformAccess();
...
// Inventory_MasterData_Alignment_Plan.md Phase 6: separate from canManageItems --
// gates cold-chain/controlled-substance controls specifically, matching the
// server-side manage-compliance permission the backend already enforces.
const canManageCompliance = ref(false);
...
function countCardValue(value: number | string | null | undefined): number {
    const numeric = Number(value ?? 0);
    return Number.isFinite(numeric) ? numeric : 0;
}

const tabCountCards = computed<Array<{ label: string; value: number }>>(() => {
    if (activeTab.value === 'inventory') {
        return [
            { label: 'Out of stock', value: countCardValue(itemCounts.value.outOfStock) },
            { label: 'Low stock', value: countCardValue(itemCounts.value.lowStock) },
            { label: 'Healthy', value: countCardValue(itemCounts.value.healthy) },
            { label: 'Total', value: countCardValue(itemCounts.value.total) },
        ];
    }
    if (activeTab.value === 'ledger') {
        return [
            { label: 'Receive', value: countCardValue(stockLedgerSummary.value.receive) },
            { label: 'Issue', value: countCardValue(stockLedgerSummary.value.issue) },
            { label: 'Adjust', value: countCardValue(stockLedgerSummary.value.adjust) },
            { label: 'Transfer', value: countCardValue(stockLedgerSummary.value.transfer) },
        ];
    }
    return [
        { label: 'Departments', value: countCardValue(departmentStockSummary.value.departments) },
        { label: 'Items', value: countCardValue(departmentStockSummary.value.items) },
        { label: 'Issued qty', value: countCardValue(departmentStockSummary.value.totalIssuedQuantity) },
        { label: 'Rows', value: countCardValue(departmentStockSummary.value.totalRows) },
    ];
});
...
const stockMovementForm = reactive({ itemId: '', category: '', subcategory: '', movementType: 'receive', adjustmentDirection: 'increase', batchId: '', batchNumber: '', lotNumber: '', manufactureDate: '', expiryDate: '', binLocation: '', manufacturer: '', sourceSupplierId: '', sourceWarehouseId: '', destinationWarehouseId: '', destinationDepartmentId: '', quantity: '', reason: '', reasonCode: '', notes: '', occurredAt: '' });
...
// Inventory_MasterData_Alignment_Plan.md Phase 5: subcategories are now real,
// configurable master data (inventory_categories/inventory_subcategories),
// served by GET /inventory-procurement/reference-data as subcategoryOptions.
// Populated by loadReferenceData(); ITEM_SUBCATEGORY_OPTIONS/GENERAL_SUBCATEGORY_OPTIONS
// above stay as the offline/error fallback so this never regresses if the API is
// unreachable -- same "expand, don't remove the fallback yet" approach as every
// other cutover in this plan.
const apiSubcategoryOptions = ref<Record<string, SearchableSelectOption[]>>({});
...
function subcategoryOptionsForCategory(categoryValue: string | null | undefined): SearchableSelectOption[] {
    ...
    const fromApi = apiSubcategoryOptions.value[key];
...
// clinicalCatalogItems is already fetched (loadReferenceData -> GET
// /inventory-procurement/reference-data) scoped server-side to active formulary items,
// with the full shape (code, name, category, unit, metadata, codes, catalogType)
// selectClinicalCatalogItem() needs -- these were previously stubbed to permanently
// empty/false/null, which is what made the create/edit catalog-link picker unreachable
// even though the data it needs was already being loaded for other purposes
// (clinicalCatalogLabel()).
function mapClinicalCatalogItemsToOptions(items: any[]): SearchableSelectOption[] {
    return items.map((item) => ({
        value: String(item.id),
        label: item.code ? `${item.name} (${item.code})` : String(item.name ?? item.id),
        description: item.category ? formatEnumLabel(String(item.category)) : undefined,
        keywords: [item.code, item.category].filter((v: unknown): v is string => typeof v === 'string' && v.length > 0),
    }));
}
const clinicalCatalogSelectOptions = computed<SearchableSelectOption[]>(() => mapClinicalCatalogItemsToOptions(clinicalCatalogItems.value));

// Narrows the medicine picker to the chosen pharmaceutical subcategory (e.g. "antibiotics"),
// which lines up with the therapeutic `category` seeded on formulary catalog items. Falls back
// to the unfiltered list when nothing matches, so taxonomy drift never hides valid medicines.
function clinicalCatalogOptionsForSubcategory(subcategory: string): SearchableSelectOption[] {
    const normalized = subcategory.trim().toLowerCase();
    if (!normalized) return clinicalCatalogSelectOptions.value;
    const filtered = clinicalCatalogItems.value.filter(
        (item) => String(item.category ?? '').trim().toLowerCase() === normalized,
    );
    return filtered.length > 0 ? mapClinicalCatalogItemsToOptions(filtered) : clinicalCatalogSelectOptions.value;
}
...
function resetStockMovementForm(item: StockMovementLookupItem | null = null): void {
    ...
    Object.assign(stockMovementForm, { itemId: item?.id ?? '', category: item?.category ?? '', subcategory: item?.subcategory ?? '', movementType: 'receive', adjustmentDirection: 'increase', batchId: '', batchNumber: '', lotNumber: '', manufactureDate: '', expiryDate: '', binLocation: '', manufacturer: '', sourceSupplierId: '', sourceWarehouseId: '', destinationWarehouseId: '', destinationDepartmentId: '', quantity: '', reason: '', notes: '', occurredAt: currentDateTimeLocal() });
}
...
const itemDetailsTab = ref('maintenance');
...
const updateClinicalCatalogOptions = computed(() => clinicalCatalogOptionsForSubcategory(itemUpdateForm.subcategory));
// Reads the live form value (not itemDetails, the original loaded snapshot) so that picking a
// new link in the picker below locks/populates identity fields immediately, the same way
// createIdentityLockedToCatalog reacts to itemCreateForm -- not just items that were already
// linked when the sheet opened.
const updateIdentityLockedToCatalog = computed(() => Boolean(itemUpdateForm.clinicalCatalogItemId.trim()));
const updateSelectedCatalogItem = computed(() => {
    const id = itemUpdateForm.clinicalCatalogItemId.trim();
    if (!id) return null;
    const entry = clinicalCatalogItems.value.find((candidate) => candidate.id === id);
    return entry
        ? {
              catalogType: entry.catalogType ?? 'formulary_item', name: entry.name ?? '', code: entry.code ?? '',
              isControlledSubstance: Boolean(entry.isControlledSubstance), controlledSubstanceSchedule: entry.controlledSubstanceSchedule ?? '',
          }
        : null;
});
const itemUpdateValidationMessages = computed(() => {
    const messages: string[] = [];
    if (!itemUpdateForm.itemName.trim()) messages.push('Item name is required.');
    if (!itemUpdateForm.category.trim()) messages.push('Category is required.');
    if (!itemUpdateForm.unit.trim()) messages.push('Unit is required.');
    return messages;
});
const itemUpdateSubmitDisabled = computed(() => itemUpdateSubmitting.value || itemUpdateValidationMessages.value.length > 0);
...
// Some metadata fields (e.g. conversionFactor) get written back as a JSON number
// rather than a string, unlike the free-text fields stringMetadataValue handles.
function numericMetadataValue(metadata: Record<string, unknown> | null | undefined, ...keys: string[]): string {
    for (const key of keys) {
        const value = metadata?.[key];
...
function clearStaleCatalogSelectionForSubcategory(form: InventoryItemFormState, subcategory: string): void {
    const id = form.clinicalCatalogItemId.trim();
    const normalized = subcategory.trim().toLowerCase();
    if (!id || !normalized) return;
    const item = clinicalCatalogItems.value.find((entry) => entry.id === id);
    if (item && String(item.category ?? '').trim().toLowerCase() !== normalized) {
        selectClinicalCatalogItem(form, '');
    }
}

function selectClinicalCatalogItem(
    ...
    // Inventory_MasterData_Alignment_Plan.md Phase 1: genericName/dosageForm/strength
    // are now structured columns on the Clinical Catalog item (the authoritative
    // source since Phase 3 dropped their inventory_items equivalents) -- prefer
    // those over metadata, which is only ever a pre-Phase-1 fallback now.
    const genericName = String(item.genericName ?? '').trim();
    const dosageForm = String(item.dosageForm ?? '').trim() || stringMetadataValue(metadata, 'dosageForm', 'dosage_form');
    const strength = String(item.strength ?? '').trim() || stringMetadataValue(metadata, 'strength');
    // conversionFactor has no structured column yet -- still metadata-only on the
    // catalog side (Phase 4's packaging templates are the modern replacement, but a
    // formulary item may predate having one defined). CreateInventoryItemUseCase
    // already prefers this over whatever the form submits when catalog-linked, so
    // surface the catalog's number here too rather than showing a field that a
    // typed-in value would silently lose to.
    const conversionFactor = numericMetadataValue(metadata, 'conversionFactor', 'conversion_factor');
    ...
    if (item.catalogType === 'formulary_item') {
        form.genericName = genericName || (name ? genericNameFromClinicalName(name, strength) : form.genericName);
        ...
        form.conversionFactor = conversionFactor || form.conversionFactor;
        // Inventory_MasterData_Alignment_Plan.md Phase 1: cold chain is a fact about the
        // drug itself (e.g. a vaccine), defined once on the Clinical Catalog item -- pick
        // it up as a default here rather than making every facility re-decide it. Only
        // ever turns cold chain ON from the catalog, never off (a facility can still
        // have its own reason to keep it enabled beyond what the catalog says), and only
        // fills in storage conditions with the coded default when nothing is set yet --
        // the catalog's own storageConditions is free text, not the coded value this
        // field's dropdown expects, so it isn't copied in directly.
...
async function openItemDetails(item: any, tab: string = 'maintenance') {
...
// Backend rejects an update payload if it merely *contains* any of these keys when the item
// was ALREADY catalog-linked before this edit (UpdateInventoryItemUseCase::execute,
// array_intersect against $existing's link state, not the live form) -- it doesn't compare
// values, so even re-sending an unchanged value trips it. buildItemPayload() always emits
// every field (shared with create, where that's correct), so update must strip these when
// stripping is warranted. This deliberately checks itemDetails (the snapshot as loaded/last
// saved), NOT updateIdentityLockedToCatalog (which reacts to the live form so the UI can lock
// fields the instant a catalog item is picked) -- when a previously-unlinked item is being
// linked for the first time in this edit, the backend has no existing link yet, so it needs
// those catalog-derived identity fields in the payload to actually save them.
const IDENTITY_LOCKED_PAYLOAD_KEYS = ['itemName', 'genericName', 'dosageForm', 'strength', 'unit', 'dispensingUnit', 'codes', 'subcategory'] as const;

function buildItemUpdatePayload(form: InventoryItemFormState): Record<string, unknown> {
    const payload = buildItemPayload(form);
    if (!itemDetails.value?.clinicalCatalogItemId) return payload;
    for (const key of IDENTITY_LOCKED_PAYLOAD_KEYS) delete payload[key];
    return payload;
}
...
async function submitItemUpdate() {
    ...
    try {
        const response = await apiRequest<{ data: any }>('PATCH', `/inventory-procurement/items/${itemDetails.value.id}`, {
            body: buildItemUpdatePayload(itemUpdateForm),
            meta: {
...
async function submitCreateItem() {
    ...
    try {
        ...
        const response = await apiRequest<{ data: StockMovementLookupItem }>('POST', '/inventory-procurement/items', {
        ...
        await reloadAll();
        // Inventory_MasterData_Alignment_Plan.md Phase 7: bridge item creation and
        // stock receipt without merging them into one transaction -- a dismissible
        // offer, not a forced redirect. openStockMovementDialog() already knows how
        // to gate this on the opening-stock permission and pre-fill the form.
        const createdItem = response.data;
        toast.success('Inventory item created.', {
            duration: 12000,
            closeButton: true,
            action: createdItem?.id ? {
                label: 'Record opening stock',
                onClick: () => openStockMovementDialog(createdItem, 'receive'),
            } : undefined,
        });
    } catch (error) {
    ...
    try {
        ...
        await apiRequest('POST', '/inventory-procurement/stock-movements', {
            body: {
                ...
                manufacturer: stockMovementRequiresBatchReceiptFields.value ? (stockMovementForm.manufacturer.trim() || null) : null,
...
async function loadPermissions() {
    const applyResolvedPermissions = (names: Iterable<string>, hasSuperAdminAccess: boolean): void => {
        ...
        canManageCompliance.value = hasSuperAdminAccess || permissionSet.has('inventory.procurement.manage-compliance');
...
bindSupplyChainPageApi({
    canRead, canManageItems, canManageCompliance, canCreateMovement, canSetOpeningStock, canLaunchCreateItem, canLaunchStockMovement,
    ...
    selectedCreateCategory, createSubcategoryOptions, selectClinicalCatalogItem,
    ...
    updateClinicalCatalogOptions, updateIdentityLockedToCatalog, updateSelectedCatalogItem,
    itemUpdateValidationMessages, itemUpdateSubmitDisabled,
...
watch(() => itemCreateForm.subcategory, (subcategory) => {
    clearStaleCatalogSelectionForSubcategory(itemCreateForm, subcategory);
});
watch(() => itemUpdateForm.subcategory, (subcategory) => {
    clearStaleCatalogSelectionForSubcategory(itemUpdateForm, subcategory);
});
watch(() => itemSearch.q, () => {
...
onMounted(async () => {
    ...
    // Ledger and department-stock manage their own loading flags (stockLedgerLoading,
    // departmentStockLoading) independently of the inventory tab's `loading` -- fire them
    // in parallel rather than awaiting them here, so the inventory tab's skeleton/dim state
    // clears as soon as its own data is ready instead of waiting on unrelated tabs.
    void loadStockLedger();
    void loadDepartmentStock();
    // Populates category/classification/clinical-catalog-linking reference data
    // (clinicalCatalogItems, itemCategoryOptions, etc.) used by the create/edit item forms.
    // Previously this only ran from the Refresh button's reloadAll(), so those forms had
    // no clinical catalog items to link against until a user happened to hit Refresh first.
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            <Tabs :model-value="activeTab" :unmount-on-hide="false" class="flex flex-col" @update:model-value="(v) => { activeTab = v as StockControlTab; }">
                <div class="sticky top-0 z-10 bg-background/95 px-6 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/80">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div class="min-w-0 space-y-0.5">
                            <div class="flex flex-wrap items-center gap-2">
                                <h1 class="text-lg font-bold tracking-tight md:text-xl">Stock Control</h1>
                                <Badge v-if="permissionsResolved && !canRead" variant="outline" class="h-5 px-1.5 text-[10px] font-medium">
                                    View only
                                </Badge>
                            </div>
                            <p class="text-xs text-muted-foreground">Inventory items, stock ledger, and department stock</p>
                        </div>
                        <div class="flex shrink-0 flex-wrap items-center gap-2">
                            ...
                            <Tooltip>
                                <TooltipTrigger as-child>
                                    <Button variant="outline" size="sm" class="h-8 w-8 p-0" :disabled="catalogExporting" @click="handleExport">
                            ...
                            <Tooltip>
                                <TooltipTrigger as-child>
                                    <Button variant="outline" size="sm" class="h-8 w-8 p-0" :disabled="catalogPrinting" @click="handlePrint">
                            ...
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" class="w-48">
                                    <DropdownMenuItem v-if="canReconcileStock" @click="openReconcileDialog" :disabled="!canLaunchReconciliation">
                                    ...
                                    <DropdownMenuItem v-if="activeTab === 'inventory' && canSyncFromCatalog" @click="openCatalogSyncDialog">
                                        <AppIcon name="book-open" class="size-4" /> Sync from Catalog
                                    </DropdownMenuItem>
                    ...
                    <template v-if="canRead">
                        <div class="mt-3 grid grid-cols-4 gap-2">
                            <div v-for="card in tabCountCards" :key="card.label" class="rounded-md border bg-muted/50 px-2.5 py-1.5">
                                <p class="text-[10px] font-medium tracking-wider text-muted-foreground uppercase">{{ card.label }}</p>
                                <p class="text-sm font-bold tabular-nums">{{ card.value.toLocaleString() }}</p>
                            </div>
                        </div>

                        <TabsList class="mt-3 grid h-9 w-full grid-cols-3 gap-1">
                            <TabsTrigger value="inventory" class="gap-1.5 text-xs">
                                <AppIcon name="package" class="size-3" />
                                Inventory Items
                            </TabsTrigger>
                            <TabsTrigger value="ledger" class="gap-1.5 text-xs">
                                <AppIcon name="activity" class="size-3" />
                                Stock Ledger
                            </TabsTrigger>
                            <TabsTrigger value="department-stock" class="gap-1.5 text-xs">
                        ...
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            ...
                            <template v-if="activeTab === 'inventory'">
                                <Select :model-value="toSelectValue(itemSearch.category)" @update:model-value="itemSearch.category = fromSelectValue(String($event ?? EMPTY_SELECT_VALUE)); itemSearch.page = 1; refreshInventoryItems()">
                                    <SelectTrigger class="h-9 w-40 bg-background"><SelectValue placeholder="All Categories" /></SelectTrigger>
                                    <SelectContent>
                                ...
                                <Select :model-value="toSelectValue(itemSearch.stockState)" @update:model-value="itemSearch.stockState = fromSelectValue(String($event ?? EMPTY_SELECT_VALUE)); itemSearch.page = 1; refreshInventoryItems()">
                                    <SelectTrigger class="h-9 w-40 bg-background"><SelectValue placeholder="All stock states" /></SelectTrigger>
                                    <SelectContent>
                                ...
                                <Select :model-value="toSelectValue(itemSearch.sortBy)" @update:model-value="itemSearch.sortBy = fromSelectValue(String($event ?? EMPTY_SELECT_VALUE)); itemSearch.page = 1; refreshInventoryItems()">
                                    <SelectTrigger class="h-9 w-36 bg-background"><SelectValue placeholder="Sort by" /></SelectTrigger>
                                    <SelectContent>
                            ...
                            <template v-else-if="activeTab === 'ledger'">
                                <Select :model-value="toSelectValue(stockLedgerFilters.movementType)" @update:model-value="stockLedgerFilters.movementType = fromSelectValue(String($event ?? EMPTY_SELECT_VALUE)); applyStockLedgerFilters()">
                                    <SelectTrigger class="h-9 w-36 bg-background"><SelectValue placeholder="All types" /></SelectTrigger>
                                    <SelectContent>
                                ...
                                <Select :model-value="toSelectValue(stockLedgerFilters.sourceKey)" @update:model-value="stockLedgerFilters.sourceKey = fromSelectValue(String($event ?? EMPTY_SELECT_VALUE)); applyStockLedgerFilters()">
                                ...
                                <input v-model="stockLedgerFilters.from" type="datetime-local" class="h-9 rounded-lg border border-input bg-background px-3 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" @change="applyStockLedgerFilters()" />
                                <input v-model="stockLedgerFilters.to" type="datetime-local" class="h-9 rounded-lg border border-input bg-background px-3 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" @change="applyStockLedgerFilters()" />
                            </template>
                            <template v-else-if="activeTab === 'department-stock'">
                                <Select :model-value="toSelectValue(departmentStockFilters.departmentId)" @update:model-value="setDepartmentStockDepartmentFilter(String($event ?? EMPTY_SELECT_VALUE))">
                                    <SelectTrigger class="h-9 w-48 bg-background" :disabled="!canSelectAnyRequisitionDepartment">
                        ...
                        <div v-if="hasAnyStockFilter" class="mt-3 flex flex-wrap items-center gap-1.5">
                ...
                <div v-if="!permissionsResolved" class="px-6">
                    <Alert>
                ...
                <div v-else-if="!canRead" class="px-6">
                ...
                <div v-if="canRead" class="flex min-h-0 flex-1 flex-col px-6 pb-6">
                    <TabsContent value="inventory" class="m-0 flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border bg-card">
                        <SupplyChainInventoryTab />
                    </TabsContent>
                    <TabsContent value="ledger" class="m-0 flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border bg-card">
                        <SupplyChainLedgerTab />
                    </TabsContent>
                    <TabsContent value="department-stock" class="m-0 flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border bg-card">
                        <SupplyChainDepartmentStockTab />
                    </TabsContent>
                </div>
            </Tabs>

            <SupplyChainFilterOverlays v-if="canRead" />
        </div>
    </AppLayout>
                ...
                <div class="grid gap-4">
                    ...
                    <fieldset class="grid gap-2 rounded-lg border p-2 sm:grid-cols-2">
                        ...
                        <label class="flex items-center gap-2 pt-2 text-sm">
                            <Checkbox :model-value="unitForm.isDefaultSalesUnit" :disabled="unitFormSubmitting" @update:model-value="(value) => (unitForm.isDefaultSalesUnit = value === true)" />
                            Default sales unit
                        </label>
                        <label class="flex items-center gap-2 pt-2 text-sm">
                            <Checkbox :model-value="unitForm.isDefaultPurchaseUnit" :disabled="unitFormSubmitting" @update:model-value="(value) => (unitForm.isDefaultPurchaseUnit = value === true)" />
