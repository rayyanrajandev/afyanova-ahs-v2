                    ...
                    <div class="flex items-center gap-3 rounded-lg border border-sidebar-border/70 bg-card px-4 py-3 shadow-sm">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold leading-tight tabular-nums">
                                <template v-if="ws.shortageQueueMeta">{{ ws.shortageQueueMeta.total ?? ws.shortageQueueItems.length }}</template>
                    ...
                    <div class="flex items-center gap-3 rounded-lg border border-green-200/70 bg-green-50/50 px-4 py-3 shadow-sm dark:border-green-900/40 dark:bg-green-950/20">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold leading-tight tabular-nums text-green-700 dark:text-green-300">
                                <template v-if="ws.shortageQueueMeta">{{ ws.shortageQueueMeta.readyLineCount ?? 0 }}</template>
                                <template v-else>—</template>
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3 rounded-lg border border-amber-200/70 bg-amber-50/50 px-4 py-3 shadow-sm dark:border-amber-900/40 dark:bg-amber-950/20">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold leading-tight tabular-nums text-amber-700 dark:text-amber-300">
                                <template v-if="ws.shortageQueueMeta">{{ ws.shortageQueueMeta.waitingLineCount ?? 0 }}</template>
                                <template v-else>—</template>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 border-b px-4 py-3">
                        ...
                        <div class="flex rounded-lg border bg-muted/40 p-0.5 text-xs">
                            <button
                                ...
                                :class="ws.shortageQueueFilters.readiness === 'all'
                                    ? 'bg-background text-foreground shadow-sm'
                                    : 'text-muted-foreground hover:text-foreground'"
                                @click="ws.shortageQueueFilters.readiness = 'all'; ws.shortageQueueFilters.page = 1; ws.loadShortageQueue()"
                            >
                                All
                                <span
                                    v-if="ws.shortageQueueMeta"
                                    class="inline-flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] tabular-nums"
                                    :class="ws.shortageQueueFilters.readiness === 'all' ? 'bg-muted text-foreground' : 'bg-muted/60 text-muted-foreground'"
                                >{{ (ws.shortageQueueMeta.readyLineCount ?? 0) + (ws.shortageQueueMeta.waitingLineCount ?? 0) }}</span>
                            </button>
                            <button
                                class="flex items-center gap-1.5 rounded-md px-3 py-2 font-medium transition-all"
                                :class="ws.shortageQueueFilters.readiness === 'ready'
                                    ? 'bg-background text-green-700 shadow-sm dark:text-green-300'
                                    : 'text-muted-foreground hover:text-foreground'"
                                @click="ws.shortageQueueFilters.readiness = 'ready'; ws.shortageQueueFilters.page = 1; ws.loadShortageQueue()"
                            >
                                <span class="size-1.5 rounded-full bg-green-500"></span>
                                Ready
                                <span
                                    v-if="ws.shortageQueueMeta && ws.shortageQueueMeta.readyLineCount > 0"
                                    class="inline-flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] tabular-nums"
                                    :class="ws.shortageQueueFilters.readiness === 'ready' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200' : 'bg-muted/60 text-muted-foreground'"
                                >{{ ws.shortageQueueMeta.readyLineCount }}</span>
                            </button>
                            <button
                                class="flex items-center gap-1.5 rounded-md px-3 py-2 font-medium transition-all"
                                :class="ws.shortageQueueFilters.readiness === 'waiting'
                                    ? 'bg-background text-amber-700 shadow-sm dark:text-amber-300'
                                    : 'text-muted-foreground hover:text-foreground'"
                                @click="ws.shortageQueueFilters.readiness = 'waiting'; ws.shortageQueueFilters.page = 1; ws.loadShortageQueue()"
                            >
                                <span class="size-1.5 rounded-full bg-amber-500"></span>
                                Waiting
                                <span
                                    v-if="ws.shortageQueueMeta && ws.shortageQueueMeta.waitingLineCount > 0"
                                    class="inline-flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] tabular-nums"
                                    :class="ws.shortageQueueFilters.readiness === 'waiting' ? 'bg-amber-100 text-amber-800 dark:bg-amber-900/50 dark:text-amber-200' : 'bg-muted/60 text-muted-foreground'"
                                >{{ ws.shortageQueueMeta.waitingLineCount }}</span>
                            </button>
                        </div>

                        <div class="flex min-w-0 flex-1 items-center gap-2">
                            <SearchInput
                                v-model="ws.shortageQueueFilters.q"
                                placeholder="Search requisition or department…"
                                class="min-w-0 flex-1 text-xs"
                                @keyup.enter="ws.shortageQueueFilters.page = 1; ws.loadShortageQueue()"
                            />
                            <Select
                        ...
                        <Button
                            ...
                            :disabled="ws.shortageQueueLoading"
                            @click="ws.shortageQueueFilters.page = 1; ws.loadShortageQueue()"
                        >
                            <AppIcon name="refresh-cw" :class="['size-3.5', ws.shortageQueueLoading && 'animate-spin']" />
                            Refresh
                        </Button>
                    </div>
