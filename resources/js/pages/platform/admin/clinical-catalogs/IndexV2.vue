...
type PackagingTemplate = {
    id: string;
    clinical_catalog_item_id: string;
    unit_name: string;
...
const canManageCompliance = computed(() => permissionState('inventory.procurement.manage-compliance') === 'allowed');
...
const packagingTemplatesLoading = ref(false);
const packagingTemplatesError = ref<string | null>(null);
const packagingTemplates = ref<PackagingTemplate[]>([]);
const packagingTemplateSaving = ref(false);
const packagingTemplateForm = reactive({
    unitName: '',
    unitCode: '',
    baseQuantity: '',
    isBaseUnit: false,
...
function resetPackagingTemplateWorkspace(): void {
    packagingTemplatesLoading.value = false;
    packagingTemplatesError.value = null;
    packagingTemplates.value = [];
    Object.assign(packagingTemplateForm, { unitName: '', unitCode: '', baseQuantity: '', isBaseUnit: false, isDefaultPurchaseUnit: false, isDefaultSalesUnit: false });
}

async function loadPackagingTemplates(item: Item): Promise<void> {
    const id = String(item.id ?? '').trim();
    if (itemCatalogKey(item) !== 'formulary-items' || !id) {
        resetPackagingTemplateWorkspace();
        return;
    }

    packagingTemplatesLoading.value = true;
    packagingTemplatesError.value = null;

    try {
        const response = await apiRequest<{ data: PackagingTemplate[] }>('GET', `/platform/admin/clinical-catalogs/formulary-items/${id}/packaging-templates`);
        packagingTemplates.value = response.data ?? [];
    } catch (error) {
        packagingTemplatesError.value = messageFromUnknown(error, 'Unable to load packaging templates.');
        packagingTemplates.value = [];
    } finally {
        packagingTemplatesLoading.value = false;
    }
...
async function addPackagingTemplate(): Promise<void> {
    const id = String(selected.value?.id ?? '').trim();
    if (!id || !canManage.value || packagingTemplateSaving.value) return;
    if (!packagingTemplateForm.unitName.trim() || !packagingTemplateForm.baseQuantity.trim()) return;

    packagingTemplateSaving.value = true;
    packagingTemplatesError.value = null;

    try {
        const response = await apiRequest<{ data: PackagingTemplate[] }>('POST', `/platform/admin/clinical-catalogs/formulary-items/${id}/packaging-templates`, {
            body: {
                unit_name: packagingTemplateForm.unitName.trim(),
                unit_code: packagingTemplateForm.unitCode.trim() || null,
                base_quantity: Number(packagingTemplateForm.baseQuantity),
                is_base_unit: packagingTemplateForm.isBaseUnit,
                is_default_purchase_unit: packagingTemplateForm.isDefaultPurchaseUnit,
                is_default_sales_unit: packagingTemplateForm.isDefaultSalesUnit,
            },
        });
        packagingTemplates.value = response.data ?? [];
        Object.assign(packagingTemplateForm, { unitName: '', unitCode: '', baseQuantity: '', isBaseUnit: false, isDefaultPurchaseUnit: false, isDefaultSalesUnit: false });
        notifySuccess('Packaging template added.');
    } catch (error) {
        packagingTemplatesError.value = messageFromUnknown(error, 'Unable to add packaging template.');
    } finally {
        packagingTemplateSaving.value = false;
    }
}

async function deletePackagingTemplate(templateId: string): Promise<void> {
    const id = String(selected.value?.id ?? '').trim();
    if (!id || !canManage.value) return;

    try {
        const response = await apiRequest<{ data: PackagingTemplate[] }>('DELETE', `/platform/admin/clinical-catalogs/formulary-items/${id}/packaging-templates/${templateId}`);
        packagingTemplates.value = response.data ?? [];
        notifySuccess('Packaging template removed.');
    } catch (error) {
        notifyError(messageFromUnknown(error, 'Unable to remove packaging template.'));
    }
}
    ...
    try {
        await apiRequest('POST', base.value, {
            body: {
                ...
                metadata,
                genericName: createForm.genericName.trim() || null,
                storageConditions: createForm.storageConditions.trim() || null,
                requiresColdChain: createForm.requiresColdChain,
                isControlledSubstance: createForm.isControlledSubstance,
                controlledSubstanceSchedule: createForm.controlledSubstanceSchedule.trim() || null,
                genericGroupCode: createForm.genericGroupCode.trim() || null,
            },
        });
        resetCreateForm();
        createSheetOpen.value = false;
...
async function openDetails(item: Item): Promise<void> {
    ...
    try {
        ...
        // Load packaging templates so overview card shows the reusable pack sizes
        if (itemCatalogKey(response.data) === 'formulary-items') {
            await loadPackagingTemplates(response.data);
        }
    } catch (error) {
...
async function openEditFromRow(item: Item): Promise<void> {
    const id = String(item.id ?? '').trim();
    if (!id || !canManage.value) return;

    // Opens the edit sheet directly -- does NOT go through openDetails(), which would
    // flip detailsOpen to true first and flash the details sheet before this one.
    detailsOpen.value = false;
    statusSheetOpen.value = false;
    recipeSheetOpen.value = false;
    selected.value = item;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            <div class="sticky top-0 z-10 bg-background/95 px-6 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/80">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div class="min-w-0 space-y-0.5">
                        <div class="flex flex-wrap items-center gap-2">
                            <h1 class="text-lg font-bold tracking-tight md:text-xl">Clinical Catalog</h1>
                            <Badge v-if="clinicalCatalogReadOnly" variant="outline" class="h-5 px-1.5 text-[10px] font-medium">
                                View only
                            </Badge>
                        </div>
                ...
                <Tabs v-if="canRead" :model-value="catalogKey" class="mt-3" @update:model-value="setCatalogTab">
                    <TabsList class="grid h-auto w-full grid-cols-2 gap-1 sm:grid-cols-5">
                        <TabsTrigger v-for="tab in clinicalCatalogTabs" :key="tab.key" :value="tab.key" class="gap-1 text-xs">
                ...
                <div v-if="canRead" class="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    <Select v-if="catalogKey === 'formulary-items'" :model-value="filters.dosageForm || SELECT_ALL_VALUE" @update:model-value="filters.dosageForm = $event === SELECT_ALL_VALUE ? '' : $event; search()">
                        <SelectTrigger class="h-9 w-44 bg-background">
                ...
                <div
                    ...
                    class="mt-3 flex flex-wrap items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2"
                >
                    ...
                    <Button size="sm" variant="ghost" class="h-6 px-2 text-xs" :disabled="bulkStatusBusy" @click="clearSelectedItems">
                        Clear
                    </Button>
                    <Button size="sm" variant="secondary" class="h-7 gap-1 text-xs" :disabled="bulkStatusBusy" @click="openBulkStatusDialog('active')">
                        <AppIcon name="check-circle" class="size-3" />
                        Activate
                    </Button>
                    <Button size="sm" variant="outline" class="h-7 gap-1 text-xs" :disabled="bulkStatusBusy" @click="openBulkStatusDialog('inactive')">
                        <AppIcon name="pause" class="size-3" />
                        Deactivate
                    </Button>
                    <Button size="sm" variant="destructive" class="h-7 gap-1 text-xs" :disabled="bulkStatusBusy" @click="openBulkStatusDialog('retired')">
            ...
            <div v-else-if="canRead && scopeUnresolved" class="px-6">
            ...
            <div v-if="!canRead && permissionsResolved" class="px-6 py-8 text-sm text-muted-foreground">
                Clinical catalog access is permission restricted. Request <code>platform.clinical-catalog.read</code> permission.
            </div>

            <div v-else-if="canRead" class="px-6 pb-6">
                ...
                <div v-else-if="loading || listLoading" class="overflow-hidden rounded-lg border bg-card">
                    <RegistryListSkeleton :count="6" />
                </div>

                <div v-else-if="items.length === 0" class="overflow-hidden rounded-lg border bg-card">
                    ...
                    <ul class="divide-y px-3">
                        <li v-for="item in items" :key="String(item.id)">
                            ...
                            >
                                ...
                                <template #actions>
                                    <Button v-if="canManage" size="sm" variant="outline" class="h-8 gap-1.5 rounded-lg text-xs" @click="openEditFromRow(item)">
                ...
                <SheetContent side="right" variant="workspace" size="3xl" class="flex h-full min-h-0 flex-col">
                    ...
                    <ScrollArea class="min-h-0 flex-1">
                        <div class="grid gap-4 px-6 py-4">
                            ...
                            <fieldset v-if="catalogKey === 'formulary-items'" class="space-y-3 rounded-lg border p-3">
                                <legend class="px-1 text-sm font-medium">Clinical classification &amp; compliance</legend>
                                <div class="grid gap-3 md:grid-cols-2">
                                    <div class="grid gap-1.5">
                                        <Label>Generic name</Label>
                                        <Input v-model="createForm.genericName" placeholder="e.g. Amoxicillin" />
                                    ...
                                    <div class="grid gap-1.5">
                                        ...
                                        <Input v-model="createForm.genericGroupCode" placeholder="e.g. AMOXICILLIN" />
                                        <p class="text-xs text-muted-foreground">Links brand/generic variants of the same drug together</p>
                                    </div>
                                    <div class="grid gap-1.5 md:col-span-2">
                                        <Label>Storage conditions</Label>
                                        <Input v-model="createForm.storageConditions" placeholder="e.g. Store below 25°C, protect from light" />
                                    </div>
                                </div>
                                <div class="grid gap-3 md:grid-cols-2">
                                    ...
                                    <div v-if="createForm.isControlledSubstance" class="grid gap-1.5 md:col-span-2">
                                        <Label>Controlled substance schedule</Label>
                                        <Input
                                            v-model="createForm.controlledSubstanceSchedule"
                                            :disabled="!canManageCompliance"
                                            placeholder="e.g. Schedule II"
                                        />
                                        <p v-if="firstError(createErrors, 'controlledSubstanceSchedule')" class="text-xs text-destructive">{{ firstError(createErrors, 'controlledSubstanceSchedule') }}</p>
                                    </div>
                                </div>
                            </fieldset>
                        ...
                        <Tabs v-else-if="selected" v-model="detailsSheetTab" class="flex h-full min-h-0 flex-col">
                            ...
                            <ScrollArea class="min-h-0 flex-1" viewport-class="pb-6">
                                <TabsContent value="overview" class="m-0 space-y-5 p-5">
                                    ...
                                    <!-- Packaging Templates (formulary items only) -->
                                    <div v-if="selectedCatalogKey === 'formulary-items'" class="rounded-lg border bg-background/70 px-3 py-2.5">
                                        <div class="flex items-center justify-between gap-2">
                                            <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Packaging templates</p>
                                            <Badge variant="outline">{{ packagingTemplates.length }} unit{{ packagingTemplates.length === 1 ? '' : 's' }}</Badge>
                                        </div>
                                        <p class="mt-1 text-xs text-muted-foreground">
                                            Reusable pack sizes for this drug (e.g. "blister of 10", "box of 10 blisters"). Copied into a
                                            facility's own packaging the first time an inventory item links to this catalog entry.
                                        </p>
                                        <div class="mt-2 space-y-3">
                                            <p v-if="packagingTemplatesLoading" class="text-xs text-muted-foreground">Loading packaging templates…</p>
                                            <p v-else-if="packagingTemplatesError" class="text-xs text-destructive">{{ packagingTemplatesError }}</p>
                                            <div v-else-if="packagingTemplates.length > 0" class="space-y-1.5">
                                                <div
                                                    v-for="template in packagingTemplates"
                                                    :key="template.id"
                                                    class="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-background px-3 py-2 text-xs"
                                                >
                                                    <div class="flex min-w-0 flex-1 flex-wrap items-center gap-x-3 gap-y-1">
                                                        <span class="font-medium text-foreground capitalize">{{ template.unit_name }}</span>
                                                        <span v-if="template.unit_code" class="font-mono text-[10px] text-muted-foreground">{{ template.unit_code }}</span>
                                                        <span class="text-muted-foreground">Base qty {{ template.base_quantity }}</span>
                                                        <Badge v-if="template.is_base_unit" variant="secondary" class="text-[10px]">Base unit</Badge>
                                                        <Badge v-if="template.is_default_purchase_unit" variant="outline" class="text-[10px]">Default purchase</Badge>
                                                        <Badge v-if="template.is_default_sales_unit" variant="outline" class="text-[10px]">Default sales</Badge>
                                                    </div>
                                                    <Button
                                                        v-if="canManage"
                                                        variant="ghost"
                                                        ...
                                                        title="Remove packaging template"
                                                        @click="deletePackagingTemplate(template.id)"
                                                    >
                                                        <AppIcon name="trash-2" class="size-3.5" />
                                                    </Button>
                                                </div>
                                            </div>
                                            <p v-else class="text-sm text-muted-foreground">No packaging templates yet.</p>

                                            <div v-if="canManage" class="grid gap-2 rounded-md border border-dashed p-2.5 md:grid-cols-2">
                                                <div class="grid gap-1.5">
                                                    <Label class="text-xs">Unit name</Label>
                                                    <Input v-model="packagingTemplateForm.unitName" placeholder="e.g. blister" class="h-8 text-xs" />
                                                </div>
                                                <div class="grid gap-1.5">
                                                    <Label class="text-xs">Unit code</Label>
                                                    <Input v-model="packagingTemplateForm.unitCode" placeholder="Optional barcode/code" class="h-8 text-xs" />
                                                </div>
                                                <div class="grid gap-1.5">
                                                    <Label class="text-xs">Base quantity</Label>
                                                ...
                                                <div class="md:col-span-2">
                                                    <Button
                                                        ...
                                                        variant="outline"
                                                        class="gap-1.5"
                                                        :disabled="packagingTemplateSaving || !packagingTemplateForm.unitName.trim() || !packagingTemplateForm.baseQuantity.trim()"
                                                        @click="addPackagingTemplate"
                                                    >
                                                        <AppIcon name="plus" class="size-3.5" />
                                                        {{ packagingTemplateSaving ? 'Adding...' : 'Add packaging template' }}
                                                    </Button>
                                                </div>
                                            </div>
                                        </div>
