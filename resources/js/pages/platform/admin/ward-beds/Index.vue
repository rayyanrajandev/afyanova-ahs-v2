...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Platform Admin', href: '/platform/admin/ward-beds' },
    { title: 'Ward/Beds', href: '/platform/admin/ward-beds' },
];
...
const createForm = reactive({ code: '', name: '', departmentId: '', wardName: '', bedNumber: '', location: '', notes: '' });
const editOpen = ref(false);
const editLoading = ref(false);
const editTarget = ref<WardBed | null>(null);
const editForm = reactive({ code: '', name: '', departmentId: '', wardName: '', bedNumber: '', location: '', notes: '' });
...
const auditTarget = ref<WardBed | null>(null);
const auditLoading = ref(false);
const auditError = ref<string | null>(null);
const auditLogs = ref<AuditLog[]>([]);

function scrollToCreateWardBed() {
    document.getElementById('create-ward-bed')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
...
async function loadCounts() {
    ...
    try {
        const response = await apiRequest<StatusResponse>('GET', '/platform/admin/ward-beds/status-counts', {
            query: { q: filters.q.trim() || null, departmentId: filters.departmentId || null, wardName: filters.wardName.trim() || null },
        });
...
async function createItem() {
    ...
    try {
        ...
        createForm.code = ''; createForm.name = ''; createForm.departmentId = ''; createForm.wardName = ''; createForm.bedNumber = ''; createForm.location = ''; createForm.notes = '';
        filters.page = 1;
        await refreshPage();
    } catch (error) {
        notifyError(messageFromUnknown(error, 'Unable to create ward/bed resource.'));
...
function openEdit(item: WardBed) {
    ...
    editForm.wardName = item.wardName || ''; editForm.bedNumber = item.bedNumber || ''; editForm.location = item.location || ''; editForm.notes = item.notes || '';
    editOpen.value = true;
}
...
async function saveEdit() {
    ...
    try {
        await apiRequest<ItemResponse<WardBed>>('PATCH', `/platform/admin/ward-beds/${id}`, {
            body: {
                code: editForm.code.trim(), name: editForm.name.trim(), departmentId: editForm.departmentId || null,
                wardName: editForm.wardName.trim(), bedNumber: editForm.bedNumber.trim(), location: editForm.location.trim() || null, notes: editForm.notes.trim() || null,
            },
        });
        ...
        notifyError(messageFromUnknown(error, 'Unable to update ward/bed resource.'));
...
async function saveStatus() {
    ...
    try {
        await apiRequest<ItemResponse<WardBed>>('PATCH', `/platform/admin/ward-beds/${id}/status`, {
...
async function loadAudit(item: WardBed) {
    ...
    try {
        const response = await apiRequest<AuditResponse>('GET', `/platform/admin/ward-beds/${id}/audit-logs`, { query: { page: 1, perPage: 20 } });
...
function reset() { filters.q = ''; filters.status = ''; filters.departmentId = ''; filters.wardName = ''; filters.page = 1; void refreshPage(); }
        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-lg p-4 md:p-6">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="min-w-0">
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">Manage inpatient ward and bed resources.</p>
                </div>
                <div class="flex flex-shrink-0 items-center gap-2">
                    ...
                    <Button size="sm" class="h-8 gap-1.5" @click="scrollToCreateWardBed">
            ...
            <div class="flex min-w-0 flex-col gap-4">
                    ...
                    <CardHeader class="shrink-0 gap-3 pb-3">
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                            <div class="min-w-0">
                                ...
                                <CardDescription>
                                    {{ items.length }} ward/bed resources on this page · Page {{ pagination?.currentPage ?? 1 }} of {{ pagination?.lastPage ?? 1 }}
                                </CardDescription>
                            </div>
                                    ...
                                    <PopoverContent align="end" class="flex max-h-[28rem] w-[20rem] flex-col overflow-hidden rounded-md border bg-popover p-0 shadow-md">
                                        <div class="space-y-3 border-b px-4 py-3">
                                            ...
                                            <div class="grid gap-2">
                                                <Label for="wb-status-popover">Status</Label>
                                            ...
                                            <div class="grid gap-2">
                                                <Label for="wb-ward-popover">Ward Name</Label>
                                                <Input id="wb-ward-popover" v-model="filters.wardName" placeholder="e.g. Ward A, ICU..." />
                                            </div>
                                            <div class="grid gap-2">
                                                <Label for="wb-dept-popover">Department</Label>
                                            ...
                                            <div class="grid gap-2">
                                                <Label for="wb-per-page-popover">Per page</Label>
                        ...
                        <ScrollArea class="min-h-0 flex-1">
                            <div class="min-h-[12rem] space-y-2 p-4">
                                ...
                                <div v-else-if="items.length === 0" class="rounded-lg border border-dashed p-6 text-sm text-muted-foreground">
                                    No ward/bed resources found. Try adjusting your search or filters.
                                </div>
                                        ...
                                        <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                                            <div class="space-y-0.5">
                                                ...
                                                <p class="text-xs text-muted-foreground">
                                                    Department: {{ departmentLabelById(item.departmentId) }} · Ward: {{ item.wardName || 'N/A' }} · Bed: {{ item.bedNumber || 'N/A' }}
                                                </p>
                ...
                <Card v-if="auditTarget" class="rounded-lg border-sidebar-border/70">
                    ...
                    <CardContent>
                        ...
                        <div v-else-if="auditLogs.length === 0" class="flex flex-col items-center gap-2 py-8 text-center">
                            ...
                            <p class="text-sm text-muted-foreground">No audit logs found for this ward/bed resource.</p>
                        </div>
                        <div v-else class="space-y-2">
                ...
                <!-- Create Ward/Bed card -->
                <Card id="create-ward-bed" class="rounded-lg border-sidebar-border/70">
                    <CardHeader>
                        <CardTitle class="flex items-center gap-2">
                            <AppIcon name="plus" class="size-5 text-muted-foreground" />
                            Create Ward/Bed Resource
                        </CardTitle>
                        <CardDescription>Add a new ward or bed resource record.</CardDescription>
                    </CardHeader>
                    <CardContent class="space-y-4">
                        <Alert v-if="!canManage" variant="destructive">
                            <AlertTitle>Create access restricted</AlertTitle>
                            <AlertDescription>Request <code>platform.resources.manage-ward-beds</code> permission.</AlertDescription>
                        </Alert>
                        <template v-else>
                            <div class="grid gap-3 md:grid-cols-2">
                                <div class="grid gap-2">
                                    <Label for="create-wb-code">Code</Label>
                                    <Input id="create-wb-code" v-model="createForm.code" />
                                </div>
                                <div class="grid gap-2">
                                    <Label for="create-wb-name">Name</Label>
                                    <Input id="create-wb-name" v-model="createForm.name" />
                                </div>
                                <div class="grid gap-2">
                                    <Label for="create-wb-department">Department</Label>
                                ...
                                <div class="grid gap-2">
                                    ...
                                    <Input id="create-wb-ward" v-model="createForm.wardName" />
                                </div>
                                <div class="grid gap-2">
                                    <Label for="create-wb-bed">Bed Number</Label>
                                    <Input id="create-wb-bed" v-model="createForm.bedNumber" />
                                </div>
                                <div class="grid gap-2">
                                    <Label for="create-wb-location">Location</Label>
                                    <Input id="create-wb-location" v-model="createForm.location" />
                                </div>
                                <div class="grid gap-2 md:col-span-2">
                                    <Label for="create-wb-notes">Notes</Label>
                                    <Textarea id="create-wb-notes" v-model="createForm.notes" class="min-h-20" />
                                </div>
                            </div>
                            <Separator />
            ...
            <!-- Edit Ward/Bed dialog -->
            <Dialog :open="editOpen" @update:open="(open) => (editOpen = open)">
                <DialogContent size="xl">
                    <DialogHeader>
                        <DialogTitle>Edit Ward/Bed Resource</DialogTitle>
                        <DialogDescription>Update ward/bed metadata.</DialogDescription>
                    </DialogHeader>
                    <div class="grid gap-3">
                        <div class="grid gap-2">
                            <Label for="edit-wb-code">Code</Label>
                            <Input id="edit-wb-code" v-model="editForm.code" />
                        </div>
                        <div class="grid gap-2">
                            <Label for="edit-wb-name">Name</Label>
                            <Input id="edit-wb-name" v-model="editForm.name" />
                        </div>
                        <div class="grid gap-2">
                            <Label for="edit-wb-department">Department</Label>
                        ...
                        <div class="grid gap-2">
                            ...
                            <Input id="edit-wb-ward" v-model="editForm.wardName" />
                        </div>
                        <div class="grid gap-2">
                            <Label for="edit-wb-bed">Bed Number</Label>
                            <Input id="edit-wb-bed" v-model="editForm.bedNumber" />
                        </div>
                        <div class="grid gap-2">
                            <Label for="edit-wb-location">Location</Label>
                            <Input id="edit-wb-location" v-model="editForm.location" />
                        </div>
                        <div class="grid gap-2">
                            <Label for="edit-wb-notes">Notes</Label>
                            <Textarea id="edit-wb-notes" v-model="editForm.notes" class="min-h-20" />
                        </div>
                    </div>
            ...
            <Dialog :open="statusOpen" @update:open="(open) => (statusOpen = open)">
                <DialogContent variant="action" size="lg">
                    <DialogHeader>
                        ...
                        <DialogDescription>{{ statusTarget === 'inactive' ? 'Reason is required before deactivating.' : 'Confirm activation of this ward/bed resource.' }}</DialogDescription>
                    </DialogHeader>
                    <div class="space-y-3">
                        ...
                        <div v-if="statusTarget === 'inactive'" class="grid gap-2">
                            ...
                            <Textarea id="wb-status-reason" v-model="statusReason" class="min-h-20" placeholder="Required reason for deactivation" />
                        </div>
                    </div>
                    <DialogFooter class="gap-2">
