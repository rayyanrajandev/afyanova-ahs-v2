...
import BillingQueueDateFiltersSheet from './components/BillingQueueDateFiltersSheet.vue';
import BillingQueueFilterPopover from './components/BillingQueueFilterPopover.vue';
...
>(null);
const billingQueueSearchInputRef = ref<HTMLInputElement | null>(null);
const billingQueueDateFiltersSheetOpen = ref(false);
const billingQueueSortBy = ref('invoiceDate');
const billingQueueSortDir = ref<'asc' | 'desc'>('desc');
...
function focusBillingQueueSearch() {
    nextTick(() => {
        const element = billingQueueSearchInputRef.value;
        element?.focus();
        element?.select();
    });
}
        ...
        const response = await apiRequest<BillingInvoiceListResponse>(
            ...
            '/billing-invoices',
            {
                query: {
                    ...
                    sortBy: billingQueueSortBy.value,
                    sortDir: billingQueueSortDir.value,
                },
            },
        );
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        >
            ...
            <Card class="flex min-h-0 flex-1 flex-col rounded-lg border-sidebar-border/70 shadow-sm">
                <div class="flex flex-col gap-3 border-b px-4 py-3">
                    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div v-if="billingWorkspaceView === 'queue' && canReadBillingInvoices" class="flex min-w-0 items-center gap-2">
                            <div class="relative min-w-0 flex-1 lg:flex-none">
                                ...
                                <input
                                    ref="billingQueueSearchInputRef"
                            ...
                            <BillingQueueFilterPopover
                                :filter-count="activeBillingAdvancedFilterCount"
                                :status-value="statusSelectValue"
                                :sort-by="billingQueueSortBy"
                                :sort-dir="billingQueueSortDir"
                                :per-page="searchForm.perPage"
                                :compact-rows="compactQueueRows"
                                @update:status-value="updateBillingQueueStatusValue($event)"
                                @update:sort-by="billingQueueSortBy = $event; searchForm.page = 1; void reloadQueueAndSummary()"
                                @update:sort-dir="billingQueueSortDir = $event as 'asc' | 'desc'; searchForm.page = 1; void reloadQueueAndSummary()"
                                @update:per-page="setBillingResultsPerPage(Number($event))"
                                @update:compact-rows="setBillingQueueDensity($event)"
                                @apply="submitSearch"
                                @reset="resetFilters"
                                @open-date-filters="billingQueueDateFiltersSheetOpen = true"
                            />
                        </div>
                    </div>
            ...
            <div
                ...
                >
                    <div
                        v-if="activeBillingPatientFocus"
                        class="flex flex-wrap items-center justify-between gap-2 bg-primary/5 px-4 py-2 text-xs"
                    >
                        <div>
                            <p class="font-medium text-foreground">{{ activeBillingPatientFocus.label }}</p>
                            <p class="text-muted-foreground">MRN {{ activeBillingPatientFocus.number }}</p>
                        </div>
                        <div class="flex flex-wrap gap-1.5">
                            ...
                            <Button
                                ...
                                size="sm"
                                class="h-7 text-xs"
                                @click="refocusBillingPatientQueue"
                            >
                                Refocus patient
                            </Button>
                        </div>
            ...
            <BillingQueueDateFiltersSheet
                :open="billingQueueDateFiltersSheetOpen"
                :is-mobile="isMobile"
                :patient-id="searchForm.patientId"
                :currency-code="searchForm.currencyCode"
                :default-currency-code="defaultBillingCurrencyCode"
                :invoice-date-from="searchForm.from"
                :invoice-date-to="searchForm.to"
                :payment-activity-from="searchForm.paymentActivityFrom"
                :payment-activity-to="searchForm.paymentActivityTo"
                :patient-chart-queue-focus-locked="patientChartQueueFocusLocked"
                @update:open="billingQueueDateFiltersSheetOpen = $event"
                @update:patient-id="searchForm.patientId = $event"
                @update:currency-code="searchForm.currencyCode = $event"
                @update:invoice-date-from="searchForm.from = $event"
                @update:invoice-date-to="searchForm.to = $event"
                @update:payment-activity-from="searchForm.paymentActivityFrom = $event"
                @update:payment-activity-to="searchForm.paymentActivityTo = $event"
                @apply="submitSearch"
                @reset="resetFilters"
            />
        </template>
