...
function readPharmacyAuditExportRetryHandoffFromSession(): PharmacyAuditExportRetryHandoffContext | null {
    clearSensitiveSessionStorageKey(pharmacyAuditExportRetryHandoffSessionKey);
    return null;
}
...
const {
    ...
    onRestored: () => {
        ...
        if (createServerDraftId.value) {
            createMessage.value =
                'Restored your saved pharmacy draft.';
        } else {
            createMessage.value = basketCount > 0
                ? `Restored your in-progress pharmacy basket (${basketCount} ${basketCount === 1 ? 'medicine' : 'medicines'}).`
                : 'Restored your in-progress pharmacy order draft.';
        }
    ...
    try {
        ...
        if (!options?.silent) {
            createMessage.value = wasSavedDraft
                ? 'Saved your pharmacy draft. Sign it when you are ready.'
                : 'Pharmacy draft saved. Sign it when you are ready.';
...
async function discardSavedCreateDraft(): Promise<void> {
    ...
    try {
        ...
        clearPersistedPharmacyCreateDraft();
        clearCreateLifecycleMode();
        createMessage.value = 'Discarded the saved pharmacy draft.';
...
const createPharmacyWorkflowContextLabel = computed(() => {
...
const createPharmacyWorkflowContextMeta = computed(() => {
...
const createPharmacyContextStatusLabel = computed(() => {
...
const createPharmacyContextStatusVariant = computed<
    'default' | 'secondary' | 'outline' | 'destructive'
>(() => {
            ...
            >
                ...
                <CardContent class="space-y-6">
                    ...
                    <ClinicalContextBanner
                        title="Pharmacy order context"
                        description="Confirm patient, facility, and linked encounter before requesting medication dispense."
                        ...
                        :status-variant="createPharmacyContextStatusVariant"
