...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
    { title: 'Medical record archive', href: '/medical-records' },
]);
...
const handoffAppointmentId = computed(
    () =>
        initialCreateRouteContext.appointmentId.trim() ||
        queryParam('appointmentId').trim(),
);
...
function continueDraftMedicalRecord(record: MedicalRecord) {
    ...
    notifyError(
        'This draft is not linked to an encounter. Start a consultation from Appointments.',
    );
}
...
async function beginNewConsultationWorkspace(options?: {
    ...
    const queryAppointmentId = queryParam('appointmentId').trim();
    const appointmentId = queryAppointmentId || routeAppointmentId;

    if (appointmentId || openedFromAppointments) {
        if (!appointmentId) {
            router.visit(consultationEntryAppointmentsHref());
            return;
        }

        router.visit(
            encounterWorkspaceLegacyAppointmentHref(appointmentId, {
...
async function hydratePatientSummary(patientId: string) {
    ...
    try {
        ...
        // Non-blocking enrichment for list/details cards.
    } finally {
...
function encounterLifecycleTargetName(): string {
    ...
    if (encounterLifecycleTargetKind.value === 'laboratory') {
        const target = detailsEncounterLaboratoryOrders.value.find((order) => order.id === encounterLifecycleTargetId.value);
        return target?.testName?.trim() || target?.orderNumber?.trim() || 'this laboratory order';
    }

    if (encounterLifecycleTargetKind.value === 'pharmacy') {
        const target = detailsEncounterPharmacyOrders.value.find((order) => order.id === encounterLifecycleTargetId.value);
    ...
    if (encounterLifecycleTargetKind.value === 'radiology') {
        const target = detailsEncounterRadiologyOrders.value.find((order) => order.id === encounterLifecycleTargetId.value);
        return target?.studyDescription?.trim() || target?.orderNumber?.trim() || 'this imaging order';
    }

    const target = detailsEncounterTheatreProcedures.value.find((procedure) => procedure.id === encounterLifecycleTargetId.value);
    return (
...
function matchesMedicalRecordPreset(options: {
    ...
    if (expectedTab !== 'list') return false;
...
const canLaunchConsultationFromAppointments = computed(
    () => canCreateMedicalRecords.value && canReadAppointments.value,
);
...
onMounted(() => {
    ...
    const appointmentId = initialCreateRouteContext.appointmentId.trim();
    const admissionId = initialCreateRouteContext.admissionId.trim();
    const patientId = initialCreateRouteContext.patientId.trim();

    if (tab === 'new' || appointmentId || admissionId) {
        if (appointmentId) {
            router.replace(
                encounterWorkspaceLegacyAppointmentHref(appointmentId, {
                    from: initialCreateRouteContext.from || 'medical-records',
                }),
            );
            return;
        }
        ...
        if (patientId) {
            router.replace(consultationEntryAppointmentsHref(patientId));
            return;
        }

        if (tab === 'new') {
            router.replace(consultationEntryAppointmentsHref());
            return;
        }
    }
...
<template>
    <Head title="Medical record archive" />
                ...
                <div class="flex flex-col gap-4 p-4 md:flex-row md:items-center md:justify-between md:gap-6">
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0 space-y-0.5">
                            <div class="flex flex-wrap items-center gap-2">
                                <h1 class="text-base font-semibold tracking-tight md:text-lg">Medical record archive</h1>
                                ...
                                <Badge
                                    v-if="openedFromAppointments && handoffAppointmentId"
                                    variant="outline"
                            ...
                            <p class="truncate text-xs text-muted-foreground">
                                Search, review, print, and audit finalized clinical notes. Start new consultations from Appointments.
                            </p>
                    ...
                    <div class="flex flex-shrink-0 flex-wrap items-center gap-2">
                        ...
                        <Button
                            ...
                            size="sm"
                            class="h-8 gap-1.5"
                            @click="beginNewConsultationWorkspace({ preferFocusedPatient: true })"
                        >
                                ...
                                <div class="min-w-0 space-y-1.5">
                                    ...
                                    <div class="space-y-1.5">
                                        ...
                                        <CardDescription class="max-w-3xl text-sm">
                                            Search, review, and audit clinical notes with patient-safe filters and queue context.
                                        </CardDescription>
                                    </div>
                                </div>
                                                    ...
                                                    <div v-if="expandedListRowIds[record.id]" class="border-t px-3 pb-3 pt-2.5">
                                                        <div class="space-y-2.5">
                                                            ...
                                                            <div class="flex flex-wrap items-center gap-1.5 border-t pt-2.5">
                                                                <Button
                                                                    v-if="isDraftRecord(record) && canCreateMedicalRecords"
                                                                    size="sm"
                                                                    class="gap-1.5"
                                                                    @click="continueDraftMedicalRecord(record)"
                                                                >
                                                                    <AppIcon name="pencil" class="size-3.5" />
                                                                    {{ shouldUseEncounterWorkspaceForRecord(record) ? 'Open encounter' : 'Continue note' }}
                                                                </Button>
                                                                <Button size="sm" variant="outline" @click="openRecordDetailsSheet(record)">Details</Button>

                                                                <!-- Primary status action -->
                                                                <Button
                                                                    v-if="canApplyMedicalRecordStatusAction('finalized', record) && !shouldUseEncounterWorkspaceForRecord(record)"
                                                                    size="sm"
                                                                    :disabled="actionLoadingId === record.id"
                                                                    @click="finalizeRecord(record)"
                                                                >{{ actionLoadingId === record.id ? 'Updating...' : 'Finalize note' }}</Button>
                                                                <Button
                                                                    v-if="canApplyMedicalRecordStatusAction('amended', record) && !shouldUseEncounterWorkspaceForRecord(record)"
                                                                    size="sm"
                                                                    variant="outline"
                                                                    :disabled="actionLoadingId === record.id"
                                                                ...
                                                                <!-- Order care dropdown -->
                                                                <DropdownMenu v-if="record.patientId && !shouldUseEncounterWorkspaceForRecord(record) && (canOpenLaboratoryWorkflow || canOpenPharmacyWorkflow || canOpenRadiologyWorkflow || canOpenTheatreWorkflow || canOpenBillingWorkflow)">
                                                                ...
                                                                <DropdownMenu v-if="canApplyMedicalRecordStatusAction('archived', record) && !shouldUseEncounterWorkspaceForRecord(record)">
                                                                    <DropdownMenuTrigger as-child>
