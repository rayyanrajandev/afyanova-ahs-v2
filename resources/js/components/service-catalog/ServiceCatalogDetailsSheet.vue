    ...
    <Sheet :open="open" @update:open="(value) => (open = value)">
        <SheetContent side="right" variant="workspace" size="3xl">
            <SheetHeader class="shrink-0 border-b px-6 py-4 text-left pr-12">
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant="outline">Version {{ item?.versionNumber || 1 }}</Badge>
                        <Badge v-if="item" :variant="statusVariant(item.status)">{{ formatEnumLabel(item.status) }}</Badge>
                        <Badge v-if="item" :variant="tariffLifecycleVariant(item.effectiveFrom, item.effectiveTo)">
                            {{ tariffLifecycleLabel(item.effectiveFrom, item.effectiveTo) }}
                        </Badge>
                        <Badge v-if="item" :variant="item.clinicalCatalogItemId ? 'secondary' : 'outline'">{{ clinicalCatalogLinkLabel() }}</Badge>
                    </div>
            ...
            <ScrollArea class="min-h-0 flex-1">
                <div v-if="item" class="space-y-5 p-5">
                    <div class="rounded-lg border bg-primary/5 px-4 py-3.5">
                        <div class="flex items-baseline justify-between gap-4">
                            ...
                            <div class="shrink-0 text-right">
                                ...
                                <p class="mt-0.5 text-lg font-semibold">v{{ item.versionNumber || 1 }}</p>
                            </div>
                        </div>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="rounded-lg border bg-background/70 px-3 py-2.5">
                            ...
                            <p class="mt-1 truncate text-sm font-semibold">{{ item.serviceName }}</p>
                            <p class="truncate text-xs text-muted-foreground">{{ item.serviceCode || 'Code pending' }}</p>
                        </div>
                        <div class="rounded-lg border bg-background/70 px-3 py-2.5">
                            <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Department</p>
                            <p class="mt-1 truncate text-sm font-semibold">{{ item.department || item.departmentId || 'Unassigned' }}</p>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="rounded-lg border bg-background/70 px-3 py-2.5">
                            ...
                            <div class="mt-1 flex items-center gap-2">
                                ...
                                <span class="truncate text-xs text-muted-foreground">{{ item.statusReason || 'No reason recorded' }}</span>
                            </div>
                        </div>
                        <div class="rounded-lg border bg-background/70 px-3 py-2.5">
                            <p class="text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">Last updated</p>
                            <p class="mt-1 text-sm font-semibold">{{ formatDateTime(item.updatedAt) }}</p>
                            <p class="text-xs text-muted-foreground">Created {{ formatDateTime(item.createdAt) }}</p>
                        </div>
                    </div>
