...
async function submit(): Promise<void> {
    ...
    if (!composedSummary.value) {
        fieldErrors.value = {
            triageVitalsSummary: [
                'Record at least a brief vitals or intake summary before sending the patient to the provider queue.',
            ],
        };
        return;
    }
    ...
    const clinicianUserId =
        form.routingMode === 'specific_provider'
            ? Number(form.clinicianUserId || 0)
            : 0;

    if (
        form.routingMode === 'specific_provider' &&
        (!Number.isFinite(clinicianUserId) || clinicianUserId <= 0)
    ) {
        fieldErrors.value = {
            clinicianUserId: [
                'Choose the named provider who should receive this patient next.',
            ],
        };
        return;
    }
    ...
    if (form.routingMode === 'department_pool' && department === '') {
        fieldErrors.value = {
            department: [
                'Choose the clinic or department that should own this patient before completing triage.',
            ],
        };
        return;
    }
    ...
    try {
        ...
        const apiError = error as {
            payload?: { errors?: Record<string, string[]>; message?: string };
        };
        fieldErrors.value = apiError.payload?.errors ?? {};
        submitError.value =
            apiError.payload?.message ??
            messageFromUnknown(error, 'Unable to record triage.');
    }
}
</script>
        ...
        <SheetContent side="right" variant="form" size="2xl">
            <form class="contents" @submit.prevent="submit">
                ...
                >
                    ...
                    <SheetDescription>{{
                        entry?.patientName ||
                        'Vitals, intake notes, and provider routing.'
                    }}</SheetDescription>
                </SheetHeader>
                ...
                <div class="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                            <div
                                v-for="field in STRUCTURED_FIELDS"
                                :key="field.key"
                                class="space-y-1"
                            >
                                <Label
                                    :for="`triage-${field.key}`"
                                    class="text-xs"
                                    >{{ field.label }}</Label
                                >
                                <Input
                    ...
                    <div class="space-y-1.5">
                        <Label for="triage-additional-summary"
                            >Additional intake notes (optional)</Label
                        >
                        <Textarea
                            id="triage-additional-summary"
                            v-model="form.additionalSummary"
                            rows="2"
                        />
                        <p
                            v-if="fieldError('triageVitalsSummary')"
                            class="text-sm text-destructive"
                        >
                            {{ fieldError('triageVitalsSummary') }}
                        </p>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1.5">
                            <Label for="triage-category"
                                >Triage category (optional)</Label
                            >
                            <Select v-model="form.triageCategory">
                                ...
                                <SelectContent>
                                    <SelectItem value="P1"
                                        >P1 — Resuscitation</SelectItem
                                    >
                                    <SelectItem value="P2"
                                        >P2 — Emergent</SelectItem
                                    >
                                    <SelectItem value="P3"
                                        >P3 — Urgent</SelectItem
                                    >
                                    <SelectItem value="P4"
                                        >P4 — Less urgent</SelectItem
                                    >
                                    <SelectItem value="P5"
                                        >P5 — Non-urgent</SelectItem
                                    >
                                </SelectContent>
                            </Select>
                        </div>
                        <div class="space-y-1.5">
                            <Label for="triage-notes"
                                >Triage notes (optional)</Label
                            >
                            <Input
                                id="triage-notes"
                                v-model="form.triageNotes"
                                class="h-9"
                            />
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <Tabs v-model="form.routingMode">
                            <TabsList class="grid w-full grid-cols-2">
                                <TabsTrigger value="department_pool"
                                    >Department pool</TabsTrigger
                                >
                                <TabsTrigger value="specific_provider"
                                    >Named provider</TabsTrigger
                                >
                            </TabsList>
                        </Tabs>
                        ...
                        <div v-else class="space-y-1.5">
                            ...
                            <Select v-model="form.clinicianUserId">
                                ...
                                <SelectContent>
                                    <SelectItem
                                        v-for="clinician in clinicianDirectory.data.value?.filter(
                                            (c) => c.userId !== null,
                                        ) ?? []"
                                        :key="clinician.id"
                                    ...
                                    >
                                        ...
                                        <span
                                            v-if="clinician.department"
                                            class="text-muted-foreground"
                                        >
                                            — {{ clinician.department }}</span
                                        >
                                    </SelectItem>
                                </SelectContent>
                            </Select>
                            <p
                                v-if="fieldError('clinicianUserId')"
                                class="text-sm text-destructive"
                            >
                                {{ fieldError('clinicianUserId') }}
                            </p>
                        </div>
                    </div>
                </div>
