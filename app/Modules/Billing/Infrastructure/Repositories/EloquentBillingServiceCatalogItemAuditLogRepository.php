...
class EloquentBillingServiceCatalogItemAuditLogRepository implements BillingServiceCatalogItemAuditLogRepositoryInterface
{
    public function write(
        ...
        BillingServiceCatalogItemAuditLogModel::query()->create([
            'billing_service_catalog_item_id' => $billingServiceCatalogItemId,
            'action' => $action,
            'actor_id' => $actorId,
            'changes' => $changes,
            'metadata' => $metadata,
    ...
    public function listByBillingServiceCatalogItemId(
        ...
        $paginator = BillingServiceCatalogItemAuditLogModel::query()
            ->where('billing_service_catalog_item_id', $billingServiceCatalogItemId)
    ...
    private function toPagedResult(LengthAwarePaginator $paginator): array
    {
        return [
            'data' => array_map(
                static fn (BillingServiceCatalogItemAuditLogModel $log): array => $log->toArray(),
                $paginator->items(),
            ),
            'meta' => [
