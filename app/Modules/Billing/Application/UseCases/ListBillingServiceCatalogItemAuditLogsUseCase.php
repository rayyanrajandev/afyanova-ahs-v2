...
class ListBillingServiceCatalogItemAuditLogsUseCase
{
    ...
    public function execute(string $billingServiceCatalogItemId, array $filters): ?array
    {
        $item = $this->repository->findById($billingServiceCatalogItemId);
        if (! $item) {
            return null;
        }
        ...
        return $this->auditLogRepository->listByBillingServiceCatalogItemId(
            billingServiceCatalogItemId: $billingServiceCatalogItemId,
            page: $page,
            perPage: $perPage,
            query: $query,
            action: $action,
