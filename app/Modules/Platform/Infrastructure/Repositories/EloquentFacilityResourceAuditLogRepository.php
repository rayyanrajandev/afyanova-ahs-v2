...
class EloquentFacilityResourceAuditLogRepository implements FacilityResourceAuditLogRepositoryInterface
{
    public function write(
        ...
        FacilityResourceAuditLogModel::query()->create([
            'facility_resource_id' => $facilityResourceId,
            'action' => $action,
            'actor_id' => $actorId,
            'changes' => $changes,
            'metadata' => $metadata,
    ...
    public function listByResourceId(
        ...
        $paginator = FacilityResourceAuditLogModel::query()
            ->where('facility_resource_id', $facilityResourceId)
    ...
    private function toPagedResult(LengthAwarePaginator $paginator): array
    {
        return [
            'data' => array_map(
                static fn (FacilityResourceAuditLogModel $log): array => $log->toArray(),
                $paginator->items(),
            ),
            'meta' => [
