...
class ListClinicalProcedureOrderAuditLogsUseCase
{
    ...
    public function execute(string $clinicalProcedureOrderId, array $filters): ?array
    {
        $order = $this->clinicalProcedureOrderRepository->findById($clinicalProcedureOrderId);
        if (! $order) {
            return null;
        }
        ...
        return $this->auditLogRepository->listByClinicalProcedureOrderId(
            clinicalProcedureOrderId: $clinicalProcedureOrderId,
            page: $page,
            perPage: $perPage,
            query: $query,
            action: $action,
