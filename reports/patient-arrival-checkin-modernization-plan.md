...
| **6 — Frontend rebuild** | Composables (§3.4), `ArrivalHandoffSheet.vue`, `ReceptionQueueList.vue`, dead-link cleanup | 1, 4 | Low–Medium | 2–3 weeks (two large existing pages, more surface than a single-page rebuild like Patient Chart) | **Slices 1–2 done** — standalone `reception/Queue` page shipped (slice 1); `patients/Index.vue` handoff panel's two-call walk-in race fixed and `front_desk/surface.ts` dead link repointed (slice 2). Remaining slices below. |
...
Still deferred: extracting `ArrivalHandoffSheet.vue`/`useArrivalHandoff.ts` as reusable components (slice 2 fixed the correctness bug but kept the panel's markup and local state exactly where it was), and wiring `ReceptionQueueList.vue` into `appointments/Index.vue`. Both remaining pages are 8.5K–12K lines each — any further slice there should stay similarly surgical rather than attempting a wholesale rebuild in one pass.
