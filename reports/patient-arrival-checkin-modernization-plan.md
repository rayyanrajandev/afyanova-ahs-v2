...
| **6 — Frontend rebuild** | Composables (§3.4), `ArrivalHandoffSheet.vue`, `ReceptionQueueList.vue`, dead-link cleanup | 1, 4 | Low–Medium | 2–3 weeks (two large existing pages, more surface than a single-page rebuild like Patient Chart) | **Slice 1 done** — `useReceptionQueue`/`useCheckIn`/`useWalkInCheckIn` composables, `ReceptionQueueList.vue`, and a new standalone `reception/Queue` page shipped at `/reception/queue`. Remaining slices below. |
...
**Phase 6 status**: shipped as a new, standalone page rather than a `V2.vue`/legacy-fallback pair — Phase 4's queue read-model and Phase 1's walk-in registration had zero prior frontend consumers, so there was no existing page to replace or fall back to (same reasoning as `encounters/List.vue`'s precedent). Slice 1 (this change) ships:
...
- Deliberately does **not** touch `appointments/Index.vue` or `patients/Index.vue`. Deferred to later slices: extracting `ArrivalHandoffSheet.vue` from `patients/Index.vue`'s existing handoff panel, wiring `ReceptionQueueList.vue` into `appointments/Index.vue`, and `front_desk/surface.ts` dead-link cleanup.
