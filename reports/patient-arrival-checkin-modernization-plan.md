...
| **5 — Mode A→B→C automation** | `AppointmentCheckedIn` event; Mode B shadow-log listener; Mode C skeleton-record creation (opt-in) | 1, 4 | Medium — same staged-trust discipline as the encounter canonical-state work | Mode B: 3–5 days. Mode C: 1 week, only after Mode B soak | Mode B **done** (decided to start as soon as Phase 4 shipped). Mode C still has no decided default-on timing — not started |
| **6 — Frontend rebuild** | Composables (§3.4), `ArrivalHandoffSheet.vue`, `ReceptionQueueList.vue`, dead-link cleanup | 1, 4 | Low–Medium | 2–3 weeks (two large existing pages, more surface than a single-page rebuild like Patient Chart) | Decided (full `V2.vue` + legacy fallback if this proves to be a major workflow redesign, matching the Patients/Medical Records/Encounters precedent) — not started |

**Update**: Phases 0–4 and Phase 5's Mode B are implemented and merged. Phase 4 shipped as a live query rather than the `visit_queue_entries` table originally sketched in §3.2 — a separately-synced projection is exactly the two-writes-for-one-fact shape that caused C-7 in the clinical-note audit, and a live read has nothing to drift. Remaining: Phase 5's Mode C (needs a default-on timing decision from real Mode B log volume, not yet available) and Phase 6 (frontend work, not yet started — naming convention decided but the two-page rebuild itself is a separate multi-week effort).
