...
| **5 — Visit Handoff** | Re-scoped after `reports/reception-checkin-architecture-audit.md`: not a "sheet" — `PatientVisitActionsMenu.vue` (one-click OPD/emergency check-in via `useWalkInCheckIn`, the same path `reception/Queue.vue` uses) + `PatientDirectServiceDialog.vue` (`useDirectServiceRequest`) + a plain billing link | 1, 3 | Low, once re-scoped — the "highest-complexity" estimate assumed a big multi-mode sheet; the audit found the codebase had already settled check-in as a one-click, no-form action, which collapsed most of the complexity | **Done** |
| **6 — Cutover** | Feature-parity checklist (§2.1) verified against the legacy page side-by-side; `/patients` → `IndexV2`, old page moves to `/patients/legacy` | 0-5 | Medium — the actual risk moment, per §6's de-risking strategy | **Done** |
...
**Update**: Phase 5, done — but not as originally planned. Before starting it, `reports/reception-checkin-architecture-audit.md` was commissioned specifically to check whether a "Visit Handoff sheet" would reintroduce a form-centric competing pattern for check-in, given `reception/Queue.vue` (built earlier this session) and the backend's `CheckInUseCase` had already settled on a Queue-Centric model (one atomic write advances the appointment straight to the next queue stage — no intermediate "complete a form, then the queue updates" step anywhere). The audit confirmed it, so Phase 5 was re-scoped accordingly: no sheet at all for outpatient/emergency check-in — `PatientVisitActionsMenu.vue`'s one-click `DropdownMenuItem`s call the identical `useWalkInCheckIn` path `reception/Queue.vue` uses. Only `direct-services` (a genuinely different, non-queue action needing real fields) got a small `Dialog`. `billing` is a plain link; `chart` was dropped as redundant with the Summary Popover's existing "View chart." This collapsed the phase from its original "Medium-High, 1-1.5 weeks" estimate (which assumed a big multi-mode sheet) to a same-session, low-complexity build. 2 new tests, 179/179 frontend Vitest passing, no backend changes.
...
**Update**: Phase 6 shipped — cutover complete. `routes/web.php`'s `/patients` now renders `IndexV2`; `/patients/v2` stays live as an alias (no reason to break it); the pre-cutover page moved to `/patients/legacy` for rollback, matching the `patients/{id}/chart` → `ShowV2`/`patients/{id}/chart/legacy` precedent exactly (§3.3's rollout-mechanism question resolved the same way, not re-litigated).

Re-deriving the §2.1 checklist against the legacy file (not memory) surfaced two real gaps, both closed before cutover rather than deferred:
1. **Audit log CSV export** (`exportDetailsAuditLogsCsv` in the legacy page) had no equivalent on `ShowV2.vue`'s Audit tab. The backend endpoint (`GET /patients/{id}/audit-logs/export`, `PatientController::exportAuditLogsCsv`) was already live and unused by any V2 page. Added `exportPatientAuditLogsCsv()` to `usePatientAuditLogs.ts`, matching `useMedicalRecordAuditLog.ts`'s `exportCsv()` convention exactly (`window.open` to the export URL, no fetch/blob round-trip) — a small "Export CSV" button on the Audit tab. Deliberately no filter params: the tab itself is already a deliberately simplified re-scope of the legacy filter sheet (pagination only), so the export matches what the tab shows, not the legacy filter surface it doesn't have.
2. **Duplicate-match "View" link's URL-based deep link.** The legacy page's own duplicate-check card linked to `/patients?q=...` to jump to a filtered view of the matched patient — `usePatientListFilters.ts` deliberately doesn't restore `q` from the URL (a documented, explicit non-goal), so post-cutover that link would land on `IndexV2` showing the unfiltered list. Investigated whether `IndexV2`'s own duplicate-check flow (`PatientRegistrationSheet.vue`/`PatientEditSheet.vue`) has the same gap — it doesn't; both already link a match directly to `/patients/{id}/chart`, a strictly better one-hop UX than the legacy two-hop "open filtered list, then click the row." The gap only existed inside the legacy page's own self-referential link, which would otherwise silently break for anyone using `/patients/legacy` as a rollback. Fixed by pointing that one link at `/patients/legacy?q=...` instead — a one-line, legacy-file-only fix, not a reason to add URL-restore to `IndexV2`.

Also relabeled `IndexV2.vue`'s breadcrumb/header ("Rebuild in progress…" copy removed) now that it's the live page, not a work-in-progress preview.

Verification: `php artisan wayfinder:generate --with-form` regenerated the route TS definitions (gitignored, not committed) — first attempt without `--with-form` silently diverged from `vite.config.ts`'s `wayfinder({ formVariants: true })` convention and introduced 10 spurious TS errors across unrelated auth/settings pages; regenerating with the correct flag brought the count back to the exact 778-error pre-existing baseline (zero diff, confirmed via `git stash`/`stash pop` comparison). 179/179 Vitest passing. Full Pest suite: 1499 passed, 44 failed — the same pre-existing Inventory/Pos/Pharmacy/Billing baseline this session has confirmed unchanged throughout, not a regression.
