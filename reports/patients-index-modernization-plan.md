...
| **2 — Registration + duplicate detection** | `usePatientRegistration`, draft autosave, offline queue wiring | 0 | Medium — depends on §5's dedup-scoring decision | **Slice 1 done** (core registration + live server-backed duplicate check); draft autosave + offline queue wiring deferred to a slice 2 |
