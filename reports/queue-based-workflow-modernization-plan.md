...
**Phase 1b — direct-service walk-ins were a real blind spot, closed.** Reviewing `patients/Index.vue`'s "Direct services" handoff mode (user-prompted) surfaced a fourth patient-flow pathway this plan hadn't accounted for: `POST /service-requests` lets a patient who needs only a lab/pharmacy/radiology/theatre service — no doctor visit — go straight to that department's queue (`walk-in-service-requests/Index.vue`, a real, fully-built page), bypassing triage/consultation entirely. Once picked up, the ticket can be linked to a real clinical order (`LinkServiceRequestToClinicalOrderUseCase`), at which point it enters the exact same order lifecycle Phases 0–3 already instrument. Before being linked, though, a `ServiceRequestModel` row — often with no `appointment_id` at all — was invisible to `GetActiveVisitJourneyUseCase` and the board, exactly the kind of blind spot this whole plan exists to eliminate.

This is not discarded — it's a legitimate, distinct clinical workflow real hospital systems support, kept as-is. `GetActiveVisitJourneyUseCase` now treats `ServiceRequestModel` as a fourth data source: any `PENDING`/`IN_PROGRESS`, not-yet-`linked_order_id` request becomes its own board entry (`appointmentId` nullable, keyed by `serviceRequestId` when there's no appointment), deriving `waiting_direct_service`/`in_direct_service` — deliberately new steps, not folded into `waiting_lab`/`in_lab`, since a direct-service patient never saw a clinician at all and conflating the two would misrepresent where they actually are. Once a ticket is linked, responsibility shifts to the real order's own status (already covered) — a request with `linked_order_id` set is excluded here to avoid double-counting the same work as two board rows.
