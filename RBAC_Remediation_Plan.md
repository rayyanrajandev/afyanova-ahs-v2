...
## Phase 5 — Fix Permissions Nobody Can Actually Get — **DONE, fully live (2026-07-23)**
...
| # | Task | File(s) | Result |
|---|---|---|---|
| 5.1 | Seed `billing.insurance.read`, `billing.insurance.manage`, `billing.payments.read` to `FINANCE.CLAIMS` (and `FINANCE.OFFICER` where appropriate). | `config/roles.php` — `insurance-officer` gets all three; `accountant` (`FINANCE.OFFICER`) gets `billing.payments.read` and `billing.insurance.read` (read-only — deliberately not `.manage`, to keep NHIF claims submission/tariff-import with the specialist claims role, not the generalist accountant). | **Done in config.** Verified with 4 new tests in `tests/Feature/Billing/BillingNhifPermissionsTest.php` proving the permission strings themselves unlock the previously-inaccessible NHIF routes. |
| 5.2 | Fix the `billing.service-catalog.manage` mismatch. | `config/roles.php` | **Correction to Phase 3's record, then done.** Phase 3 claimed `billing.service-catalog.manage-identity`/`.manage-pricing` were "already granted to `FINANCE.OFFICER`/`FINANCE.CONTROLLER` in `config/roles.php`" — **that was wrong**, caught while implementing this task. All three permission variants (`.manage`, `.manage-identity`, `.manage-pricing`) were in fact only ever granted to `HOSPITAL.FACILITY.ADMIN` — the same dead legacy role-code family flagged during Phase 2 (never instantiated as a real role row). So the entire billing service-catalog *write* path was unreachable by any real role, not just gated by one orphaned permission name as Phase 3 described — a materially bigger version of the same bug. Attempted the "better" fix first (remove the dead `.manage` OR-clause from the 6 FormRequests) but reverted it after it broke 16 existing tests that construct test actors via the bare `billing.service-catalog.manage` permission directly — a more established convention than expected, and removing it would have meant updating 16 tests for a change that still wouldn't have granted the correct permissions to any role. Went with the plan's other option instead: left the 6 FormRequests untouched and granted the permissions properly — `FINANCE.OFFICER` gets `.read`/`.manage-identity` (day-to-day catalog upkeep), `FINANCE.CONTROLLER` gets `.read`/`.manage`/`.manage-pricing`/`.view-audit-logs` (pricing is financially sensitive, kept with the oversight role; `.manage` given as the full override since the controller should reasonably be able to fix anything in the catalog). |

**Important — not yet fully live:** `config/roles.php` is the source of truth, but it only takes effect in the real database once `php artisan roles:sync` runs (or the facility-provisioning flow that calls it). This was **not run** against the local Postgres database this session — it's a live, non-ephemeral database (196 tables, real data, active connections observed), and writing to it wasn't something to do without checking first. The tests above verify the permission strings correctly unlock the right routes/actions; they do not verify that a real `FINANCE.CLAIMS`/`FINANCE.OFFICER`/`FINANCE.CONTROLLER` user in the actual database has these permissions yet — that requires running `roles:sync` as a deployment step.

**`roles:sync` run against the real database — one more gap found in the process.** Running it revealed `FINANCE.CLAIMS` ended up with *zero* of its 3 new permissions, and `FINANCE.OFFICER` was missing 2 of them. Root cause: `billing.insurance.read`, `billing.insurance.manage`, and `billing.payments.read` had **never existed as rows in the `permissions` table at all** in this database — and `SyncRolesFromConfig`/`roles:sync` only looks up existing permission ids to wire into `permission_role`, it does not create missing ones (unlike the dead `RoleHierarchySeeder` deleted in Phase 4, which had its own `ensurePermissionsExist()` step). Fixed with a new migration, `database/migrations/2026_07_23_000003_seed_billing_insurance_and_payments_read_permissions.php`, that only creates the 3 missing permission rows (role grants stay `config/roles.php`'s job via `roles:sync`, not baked into the migration). Ran the migration, re-ran `roles:sync`, and confirmed directly against the database that all three roles now hold exactly the permissions specified in `config/roles.php` — no more, no less.

**Verification performed:** full-suite JUnit diff after the code change (4 new tests passing, 0 newly failing) and again after adding the migration (0 changes at all, confirming it's inert for the ephemeral SQLite test database as expected). Directly queried the real Postgres database's `permission_role` table before and after running `roles:sync` to confirm the grants actually landed, not just that the command exited successfully.

**Rollback for Phase 5:** Revert the `config/roles.php` changes and the new migration (`php artisan migrate:rollback`, cascades to remove the `permission_role` rows too since `permission_id` has `cascadeOnDelete()`); no FormRequest code was ultimately touched.

**Phase Gate 5→6: cleared.** Code, tests, migration, and the real-database sync are all done and directly verified — this phase is fully live, not just "code complete."
