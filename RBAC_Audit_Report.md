...
> **Second correction, added 2026-07-23 (during Phase 5 remediation):** the correction directly above is itself partly wrong. Its first bullet claims `billing.service-catalog.manage-identity`/`.manage-pricing` are "granted to real roles" — false. Tracing the actual grant source (not done during the Phase 3 correction) found all three `billing.service-catalog.*` permission variants (`.manage`, `.manage-identity`, `.manage-pricing`) were granted only to `HOSPITAL.FACILITY.ADMIN` — the dead legacy role-code family already flagged elsewhere in this report as never instantiated as a real role row (§ HOSPITAL.\* naming drift, noted during Phase 2). So the entire billing service-catalog *write* path was, and until the Phase 5 fix remained, unreachable by any real role — a materially bigger version of the finding than either the original audit or the first Phase 3 correction described. Fixed in Phase 5 by granting the correct permissions to `FINANCE.OFFICER`/`FINANCE.CONTROLLER` in `config/roles.php` (the FormRequests themselves were left unchanged — see `RBAC_Remediation_Plan.md` Phase 5 for why). **Lesson on top of the earlier lesson:** confirming a permission is checked somewhere is not the same as confirming it is ever actually granted to a real, live role — both halves must be traced before calling an endpoint "already protected."

> **Third addendum, added 2026-07-23 (during Phase 6 remediation):** a category of finding this audit's original manual/agent-based methodology could not have caught at all, since it requires comparing live database state against a fresh install rather than reading code: building an automated permission-usage tripwire (`App\Support\Rbac\PermissionUsageAuditor`, see `RBAC_Remediation_Plan.md` Phase 6) surfaced that **162 of the real database's 275 permission rows do not exist in a fresh `php artisan migrate` at all** — only 121 did, before the Phase 6 fix. This is not a narrow gap; it spans nearly every module in the system (admissions, appointments, billing, claims, emergency triage, inpatient ward, laboratory, medical records, pharmacy, platform administration, POS, radiology, staff, theatre). The root cause: some comprehensive permission seeder — almost certainly the `RoleHierarchySeeder` deleted in Phase 4 of this plan, given how closely the missing list matches what it would have created — was run manually against this real database at some point in its history, and no migration ever replicated that. Practically, this means every finding and fix in this report describing a permission as "seeded" or "granted to a role" was true of *this specific long-lived development database*, but would **not** have been true of a fresh production deployment, a CI environment, or a disaster-recovery restore built from migrations alone — those would have started with less than half the permission catalog this report assumed existed. Fixed by `database/migrations/2026_07_23_000005_seed_baseline_permission_catalog.php`, verified by confirming a fresh SQLite install now seeds all 275 permissions exactly matching the real database. **Lesson on top of the prior two:** even "checked in code AND granted to a real role in the real database" is not sufficient if the database itself is the product of undocumented manual history — the only fully trustworthy check is "would this work on a database built from migrations alone," which is now what the Phase 6 CI tripwire actually verifies on every test run.
