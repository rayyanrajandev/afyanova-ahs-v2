...
**Status (2026-07-25): prep started, still nowhere near the bake-period gate (flags flipped one day ago, not 2-4 weeks).** Deliberately did NOT delete anything yet. What prep work found:

- `ListBillingChargeCaptureCandidatesUseCase`'s price *resolution* for all 7 domains no longer touches the legacy string-matcher at all — `ClinicalSourceKind::isLegacyPricingRemoved()` is hardcoded `true` for every kind (this happened in an earlier session, not this one). The "legacy fallback" from the 2026-07-24 bug fix is really just "don't overwrite the candidate's already-computed price if the new resolver comes back unpriced," not a second lookup call.
- `EloquentBillingServiceCatalogItemRepository::findActivePricingByServiceCode()` / `findActivePricingByServiceCodes()` — grepped the whole `app/` tree, **zero call sites**. `ChargeableItemPricingLookup` already replaced them for the payer-contract paths. These two methods look like safe, low-risk early deletes, independent of the table-drop bake period, whenever next touching this file.
- The old admin CRUD backend (`BillingServiceCatalogController` + its ~12 use cases) and its `/billing-service-catalog/*` API + web routes are still fully wired and reachable — the frontend nav link was retired, but the routes weren't. **This is genuinely still live traffic**, not just leftover risk: `PayerContracts.vue`'s "Service price" picker (2026-07-25 session) deliberately still reads `GET /billing-service-catalog/items` as the kept legacy fallback alongside the new additive "Pricing item (new engine)" picker. The table cannot be dropped while that picker exists in its current additive form.
- Added a temporary observation hook: `BillingServiceCatalogItemModel::booted()` logs `[pricing-engine-phase5-bake]` on every row read via the `retrieved` model event (catches every call site, not just grep-found ones). Once the bake period actually elapses, check these logs for real remaining callers before touching anything — remove the hook itself as part of the eventual deletion.
